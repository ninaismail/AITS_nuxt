...
<template>
    <section class="w-11/12 h-full mx-auto 2xl:w-8/12 lg:w-10/12">
        ...
        <!-- Mobile -->
        <p v-show="openDetails === key"  v-for="(item, key) in data" :key="key" class="font-[200] max-sm:text-[14px] text-justify text-accent2 pb-10 mt-10 md:hidden">{{ item.description }}</p>
    </section>
</template>