...
<template>
    <section :id="sectionid" class="w-11/12 pt-40 mx-auto 2xl:w-8/12 lg:w-10/12">
      ...
      <div class="flex flex-wrap items-center justify-center w-full gap-6 mt-10 lg:flex-row lg:gap-3 lg:justify-between" :class="{'lg:flex-row-reverse' : reverse}"
      v-motion
        :initial="{
          opacity: 0,
          y: 100,
        ...
        <div class="w-full lg:w-6/12">
          ...
          <NuxtLink v-if="item.to" :id="'go-to-' + item.title + '-page'" :aria-label="'go to ' + item.title" :to="item.to" class="cursor-pointer px-4 py-3 w-fit block max-sm:text-[14px] font-[200] text-center rounded-[8px] text-accent1 bg-bg2 hover:brightness-125">Browse</NuxtLink>
        </div>
      </div>
    </section>
</template>